...
#[derive(Debug)]
pub struct CollectionListState<'a> {
    selected: Option<usize>,
    pub(super) items: &'a [Collection],
    scroll: usize,
}

impl<'a> CollectionListState<'a> {
    pub fn new(items: &'a [Collection]) -> Self {
        CollectionListState {
            selected: None,
            items,
            scroll: 0,
    ...
    pub fn set_items(&mut self, items: &'a [Collection]) {
        self.items = items;
    }
}
